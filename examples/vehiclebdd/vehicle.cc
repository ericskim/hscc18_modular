...
using input_type = std::array<double,2>;
...
scots::SymbolicSet vehicleCreateStateSpace(Cudd &mgr) {
  ...
  double lb[state_dim]={0,0,-M_PI-0.4};  
  /* upper bounds of the hyper rectangle */
  double ub[state_dim]={10,10,M_PI+0.4}; 
  /* grid node distance diameter */
  double eta[state_dim]={.2,.2,.2};   


  scots::SymbolicSet ss(mgr,state_dim,lb,ub,eta);
...
scots::SymbolicSet vehicleCreateInputSpace(Cudd &mgr) {
  ...
  /* lower bounds of the hyper rectangle */
  double lb[input_dim]={-1,-1};  
  /* upper bounds of the hyper rectangle */
  double ub[input_dim]={1,1}; 
  /* grid node distance diameter */
  double eta[input_dim]={.3,.3};   

  scots::SymbolicSet is(mgr,input_dim,lb,ub,eta);
  is.addGridPoints();
