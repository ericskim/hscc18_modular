...
int main(){
  ...
  for(size_t i=0; i<is.getN(); i++) {
    ...
    std::cout << "input: " << param->u1 << "x" << param->u2;
    ...
    if(!i)
      ...
      for(size_t j=0; j<state_dim; j++) 
        yy[j]=interval(vnodelp::min(inf(yy[j]),inf(y[j])),vnodelp::max(sup(yy[j]),sup(y[j])));
    }

  }
  std::cout << "Overall enclosure " << yy[0] << "x" << yy[1] << "x" << yy[2] << std::endl;
