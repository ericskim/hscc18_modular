...
//  template<class T=std::array<double,1>, class U=std::array<double,1>>
  ...
  static void readFromFile(TransitionSystem* ts, const char* filename) {
    ...
    ts->noPre_=(short**)calloc(ts->N_,sizeof(short*));
    ts->noPost_=(short**)calloc(ts->N_,sizeof(short*));
    short** tempPointer=(short**)calloc(ts->N_,sizeof(short*));
    for(size_t i=0; i<ts->N_; i++) {
    ...
    while(!file.eof()) {
      ...
      if(line.find("#transition relation")!=std::string::npos) {
        ...
        for(size_t v=0;v<ts->T_;v++) {
          ...
          sline >> k;
          if(!ts->noPre_[k]) {
            ts->noPre_[k]=(short*)calloc(ts->M_,sizeof(short));
            ts->prePointer_[k]=(state_size_t*)calloc(ts->M_,sizeof(state_size_t));
            tempPointer[k]=(short*)calloc(ts->M_,sizeof(short));
          }
          ts->noPre_[k][j]++;

          if(!ts->noPost_[i]) {
            ts->noPost_[i]=(short*)calloc(ts->M_,sizeof(short));
