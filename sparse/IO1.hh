...
static void readFromFile(TransitionSystem& ts, const char* filename) {
    ...
    for(size_t i=0; i<ts.N_; i++) {
        for(size_t j=0; j<ts.M_; j++) {
            if(ts.noPre_[i] && ts.noPre_[i*ts.M_+j]!=0) {
                ts.prePointer_[i*ts.M_+j]=sumPre;
                sumPre=sumPre+ts.noPre_[i*ts.M_+j];
            }
            if(ts.noPost_[i] && ts.noPost_[i*ts.M_+j]!=0) {
                ts.postPointer_[i*ts.M_+j]=sumPost;
                sumPost=sumPost+ts.noPost_[i*ts.M_+j];
            }
        }
    }
    ...
    while(!file.eof()) {
        ...
        if(line.find("#transition relation")!=std::string::npos) {
            ...
            for(size_t v=0; v<ts.T_; v++) {
                ...
                sline >> j;
                sline >> k;
                size_t pos=ts.prePointer_[k*ts.M_+j]+tempPointer[k*ts.M_+j];
                tempPointer[k*ts.M_+j]++;
                ts->pre_[pos]=i;
            }
        }
    }
