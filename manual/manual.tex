...
%\begin{lemma}
%Let $S_2=(X_2,U_2,F_2)$ be a finite simple system and let $D$ be the output of
%Alg.~\ref{a:safety}. Then $Y_\infty:=D^{-1}(U_2)$ is the maximal fixed point
...
\begin{algorithm}[h]
  ...
  \begin{algorithmic}[1]
    \Input{ $Z_2$, $S_2=(X_2,U_2,F_2)$}
    \State {$V := \infty$}\Comment{value function}
    ...
    \EndFor
    \While {$Q \not= \emptyset$}
      \State {$x_2' :\in  Q $} \Comment{remove oldest element}
