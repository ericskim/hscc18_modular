...
ODE solvers. See also Section~\ref{s:usage:abs} for more details on how to
...
\cup_{x_2\in \bar X_2} x_2$, then we set again 
\begin{IEEEeqnarray}{c}
  F_2(x_2,u):=\emptyset.
\end{IEEEeqnarray}
...
If we would like to be robust against measurement errors $P(x)=x+\segcc{-z,z}$,
we require $Q\circ P$ to be a feedback refinement relation from $S_1$ to $S_2$.
That is simply achieved by computing $R$ according to
\begin{IEEEeqnarray}{c}
...
\end{IEEEeqnarray}
See Section~\ref{s:usage:abs} on how to account for
measurement errors in {\tt SCOTS} and~\cite[Thm.~III.5]{WeberRunggerReissig17}
...
to compute an approximation of $\varphi(\tau,c,u)$ as well as $\beta(\tau,u)$,
see Section~\ref{s:usage:abs} for details.
