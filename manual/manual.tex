...
%}%
\lstset{emph=[1]{%
      uint32_t,abs_type,input_type,state_type,%
...
To obtain the matrix $L(u)$, we maximize the partial derivatives of $f(\cdot,u)$ over the set $K'$
...
SCOTS} provides the class {\tt StaticController}. The class can be instantiated with two {\tt UniformGrids} and a {\tt
WinningDomain} according to
\begin{lstlisting}[basicstyle=\small\ttfamily]
