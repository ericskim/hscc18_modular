...
%propositions. Although, in terms of computation times and memory usage
%(see~Section~\ref{s:performance}) it is not advisable to use BDDs as underlying data
%structure in the computation of symbolic models, there are several situations in which BDD representations are useful.
%For example,
%\begin{enumerate}
%  \item to use {\tt SCOTS} in combination with other tools, such as {\tt slugs};
%  \item to compactly save atomic propositions and controllers to a file;
%  \item to write customized synthesis algorithms;
%\end{enumerate}
%Representations
%\begin{enumerate}
%  \item transition function $F_2$ as BDD
%  \item synthesis
%  \item writing BDDs to file
%\end{enumerate}
%
%
%
%
%It might be desirable to obtain a

%\section{MATLAB Interface}
%
%See the MATLAB commands {\tt StaticController} and {\tt GridPoints} 
