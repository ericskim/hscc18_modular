...
An \emph{invariance} specification for $S_1$ associated with $I_1,Z_1$ follows by 
\begin{IEEEeqnarray*}{c}
...
Similar to the case of the invariance specification, the set $D^{-1}(U_2)$ is
...
\begin{IEEEeqnarray*}{c}
  ...
  \big\}.
\end{IEEEeqnarray*}
%A fixed point characterization of the winning region
%
%
%In case that the user is familiar with usual game formulation and fixed point
%characterization of synthesis algorithms as for example described in~\cite{BloemJobstmannPitermanPnueliSaar12}, it is rather straightforward 
