 ...
 * USA.
 *
 * $Id: HighPrec.C 592 2009-01-28 10:04:19Z christian $
 *
 ************************************************************************/

static char rcs_id[] = "$Id: HighPrec.C 592 2009-01-28 10:04:19Z christian $";
...
class HIGHPREC_INIT {
  static USHORT i;
public:
  HIGHPREC_INIT () { if (0 == i++) { LrInit (); AccuInit (); } }
  ~HIGHPREC_INIT () { if (0 == --i) FreeAccu (); };
};

USHORT HIGHPREC_INIT::i = 0;
...
VOID FreeAccu ()
{
  LrDeleteTemp(laccu1, ACCU_DIGITS, 3);
  LrDeleteTemp(laccu2, ACCU_DIGITS, 3);
  LrDeleteTemp(laccu3, ACCU_DIGITS, 3);
  LrDeleteVar(lprod1, PRODUCT_DIGITS);
  LrDeleteVar(lprod2, PRODUCT_DIGITS);
  LrDeleteTemp(lprod, PRODUCT_DIGITS, 3);
  LiDeleteTemp(liaccu1, ACCU_DIGITS, 3);
  LiDeleteTemp(liaccu2, ACCU_DIGITS, 3);
  LiDeleteTemp(liaccu3, ACCU_DIGITS, 3);
  LiDeleteTemp(liprod, PRODUCT_DIGITS, 3);
  LiDeleteTemp(lifac1, PRODUCT_DIGITS, 3);
  LiDeleteTemp(lifac2, PRODUCT_DIGITS, 3);
  LrDeleteTemp(lwrk, PRODUCT_DIGITS, 3);
}
