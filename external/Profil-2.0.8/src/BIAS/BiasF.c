 ...
 * USA.
 *
 * $Id: BiasF.c 592 2009-01-28 10:04:19Z christian $
 *
 ************************************************************************/
...
static const char rcs_id[] = "$Id: BiasF.c 592 2009-01-28 10:04:19Z christian $";
  ...
  if (x_inf < 0.0) _BiasError ("Power: Base is negative");
	if (x_inf == 0.0) {
		if (BiasInf(pY) <= 0.0) {
			...
			return;
		} else if ((x_sup = BiasSup(pX)) == 0.0) {
			BiasHullRR(pR, &Zero, &Zero);
			return;
		} else {
      BiasHullR (&t2, &x_sup);
      BiasLog (&t1, &t2);
