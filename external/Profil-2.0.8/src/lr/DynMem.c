...
PVOID MemStore [(MEM_MAXSIZE / sizeof (PVOID)) + 1];
  ...
  */
{
  ...
  for (Size = 0; Size <= MEM_MAXSIZE; Size += sizeof (PVOID)) {
...
PVOID _MemNew (SIZE Size)
  ...
  *  Annahmen : Size ist ohne Rest durch sizeof (PVOID) teilbar.
...
PVOID _MemResize (PVOID Memory, SIZE OldSize, SIZE NewSize)
  ...
  *		Speicherbereichs.
  *  Annahmen : OldSize und NewSize sind ohne Rest durch sizeof (PVOID) teilbar.
...
{
  PVOID NewMemory;
  SIZE	OldSize = (*Size + sizeof (PVOID) - 1) & ~(sizeof (PVOID) - 1);
  SIZE	NewSize = *Size == 0 ? 
		(*Size = sizeof (PVOID)) : ((*Size <<= 1) + sizeof (PVOID) - 1) & ~(sizeof (PVOID) - 1);
  ...
  */
{
  ...
  OldSize  = (( *nElmts	* ElmtSize) + sizeof (PVOID) - 1) & ~(sizeof (PVOID) - 1);
  NewSize  = (((*nElmts == 0 ? (*nElmts = 1) : (*nElmts <<= 1))
      * ElmtSize) + sizeof (PVOID) - 1) & ~(sizeof (PVOID) - 1);
  ...
  */
{
  ...
  if ((Array = (PPVOID) _MemNew (nRows * sizeof (PVOID))) != NULL) {
    SIZE Size = ((nColumns * AlignedSize (ElmtSize)) + sizeof (PVOID) - 1) & ~(sizeof (PVOID) - 1);
